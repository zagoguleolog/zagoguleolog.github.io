...
Страница [`app/template-synth.html`](../app/template-synth.html) (`http://127.0.0.1:4173/app/template-synth.html`) — полноценный генератор тона на том же движке и [`tone-gen-ui-shared.mjs`](../app/tone-gen-ui-shared.mjs), с префиксом id **`tpl-`**. Крутилки, фейдер и **матрица** частичных тонов n=1…16 из synth-kit синхронизированы со скрытыми полями формы, которые читает `readToneGenParams`.
