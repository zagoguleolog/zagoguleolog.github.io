...
На [`app/seventh-chords-mobile.html`](../app/seventh-chords-mobile.html) используется мобильная fullscreen-компоновка с префиксом **`sc7m-ntg-`**: сверху узкая строка **Звук** с четырьмя видимыми контролами (**громкость**, **угасание**, **смесь обертонов**, **детюн**), слева вертикальный столбец шести кнопок типов **септаккорда**, в центре экранная клавиатура, снизу выбор вида клавиатуры. Форма колебания, опорная частота **A4**, диапазон октав, **спад** и матрица «Обертоны» остаются скрытыми полями/узлами для контракта `readToneGenParams`; матрица получает дефолтные частичные тоны n=1…6. **Угасание** на этой странице по умолчанию — **30 мс**.
