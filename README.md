...
В настройках репозитория: **Source → Deploy from a branch**, ветка **main**, папка **`/ (root)`** (корень репо — каталог `music/` при отдельном репозитории только для сайта). Ожидаемый URL: `https://<user>.github.io/<repo>/`. Точка входа для пользователя: **`index.html`** (краткая витрина основных интерактивных страниц); полная карта сайта — **`web/stranichki.html`**. Навигация (`site-nav.mjs`) строит ссылки относительно каталога `music/`, чтобы меню работало с префиксом репозитория на Pages; ссылка «Главная» ведёт на **`index.html`**. Интерактивные страницы открываются в браузере по HTTPS — отдельно ничего устанавливать не нужно.
