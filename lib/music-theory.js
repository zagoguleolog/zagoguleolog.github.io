...
/**
 * Номера ступеней тональности (1…7) для трёх звуков триады сектора круга
 * (мажорная или минорная от корня подписи) в выбранной тональности.
 * @param {string} tonicName
 * @param {'major' | 'naturalMinor'} keyMode
 * @param {number} rootPc pitch class корня триады 0…11
 * @param {'major' | 'minor'} triadQuality качество триады (внешнее / внутреннее кольцо)
 * @returns {number[] | null} три номера ступеней по возрастанию или null, если аккорд не диатонический
 */
export function diatonicTriadDegreeNumbersInKey(tonicName, keyMode, rootPc, triadQuality) {
  if (rootPc == null || rootPc < 0 || rootPc > 11) return null;
  if (triadQuality !== 'major' && triadQuality !== 'minor') return null;
  const patternId = keyMode === 'major' ? 'major' : 'naturalMinor';
  const inst = buildScale(patternId, tonicName);
  /** @type {Map<number, number>} PC → номер ступени 1…7 */
  const pcToDegree = new Map(inst.degrees.map((d) => [d.pc, d.index + 1]));
  const third = triadQuality === 'major' ? 4 : 3;
  const triadPcs = [rootPc, normalizePc(rootPc + third), normalizePc(rootPc + 7)];
  const nums = triadPcs.map((pc) => pcToDegree.get(pc));
  if (nums.some((n) => n == null)) return null;
  return nums.sort((a, b) => a - b);
}

/** Русские функциональные имена ступеней I…VII (натуральный мажор / нат. минор). */
const DIATONIC_FUNCTION_RU_BY_DEGREE = Object.freeze([
  'тоника',
  'супертоника',
  'медианта',
  'субдоминанта',
...
/** Качество диатонической триады на ступени 1…7. */
const MAJOR_KEY_TRIAD_QUALITY = Object.freeze(['M', 'm', 'm', 'M', 'M', 'm', 'dim']);
const NATURAL_MINOR_KEY_TRIAD_QUALITY = Object.freeze(['m', 'dim', 'M', 'm', 'm', 'M', 'M']);
const ROMAN_BASE = Object.freeze(['I', 'II', 'III', 'IV', 'V', 'VI', 'VII']);

/**
 * Римская цифра диатонической триады на ступени 1…7 в выбранной тональности.
 * @param {number} degreeNumber 1…7
 * @param {'major' | 'naturalMinor'} keyMode
 */
function romanNumeralForDiatonicDegree(degreeNumber, keyMode) {
  const qualities = keyMode === 'major' ? MAJOR_KEY_TRIAD_QUALITY : NATURAL_MINOR_KEY_TRIAD_QUALITY;
  const q = qualities[degreeNumber - 1];
  const base = ROMAN_BASE[degreeNumber - 1];
  if (q === 'M') return base;
...
/**
 * Ступень диатонической триады по корню сектора круга в выбранной тональности:
 * римская цифра (I, ii, IV …) и функциональное имя (тоника, субдоминанта …).
 * @param {string} tonicName
 * @param {'major' | 'naturalMinor'} keyMode
 * @param {number} rootPc pitch class корня 0…11
 * @returns {{ degreeNumber: number, roman: string, functionRu: string } | null}
 */
export function diatonicTriadDegreeLabelInKey(tonicName, keyMode, rootPc) {
  if (rootPc == null || rootPc < 0 || rootPc > 11) return null;
  const patternId = keyMode === 'major' ? 'major' : 'naturalMinor';
  const inst = buildScale(patternId, tonicName);
  const deg = inst.degrees.find((d) => d.pc === rootPc);
  if (!deg) return null;
  const degreeNumber = deg.index + 1;
  return {
    degreeNumber,
    roman: romanNumeralForDiatonicDegree(degreeNumber, keyMode),
    functionRu: DIATONIC_FUNCTION_RU_BY_DEGREE[deg.index],
  };
}
