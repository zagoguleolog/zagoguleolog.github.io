#087 | 2026-06-20 00:01:23 — DONE | Интеграция vAIbe-OS: submodule `repositories/music`, удалён `Проекты/music/`; `docs/AI_RULES.md` + stub `docs/system/*`; README. Проверка: `npm run verify` OK.

#086 | 2026-06-17 — DONE | Круг: римские ступени (I…vii°) и функции (тоника, субдоминанта …) на **зелёных** секторах диатоники 6+2, не на синем выделении; `diatonicTriadDegreeLabelInKey`, `syncInScaleDegreeLabels`, стили `cof-degree-roman` / `cof-degree-function`. Документация и подсказки HTML. Проверка: `npm run verify`, `node --check app/circle-scales.mjs` OK.

#085 | 2026-06-17 — DONE | Круг `circle-scales`: на выбранном секторе — номера ступеней (1…7) триады в текущей тональности (`diatonicTriadDegreeNumbersInKey`, `syncSelectedSectorDegreeLabels`, стили `cof-degree-num`); обновлены `index.html`, `README.md`, `docs/overview.md`, `web/stranichki.html`. GitHub Pages: **https://zagoguleolog.github.io/app/circle-scales.html**. Проверка: `npm run verify`, `node --check app/circle-scales.mjs` OK. *(заменено #086 — подписи на зелёных секторах)*

#084 | 2026-04-27 — DONE | `app/seventh-chords-mobile.css` / `.mjs`: fullscreen-страница септаккордов теперь сохраняет landscape-компоновку даже при portrait-положении телефона через поворот корневой рабочей области; выбор типа септаккорда на телефоне переведён на `pointerdown` / `touchstart` с `click` как fallback. Корневые причины записаны в ERR-009 и ERR-010. Проверка: `node --check app/seventh-chords-mobile.mjs`, `npm run verify` OK.

#083 | 2026-04-27 — DONE | `app/seventh-chords-mobile.html` / `.css` / `.mjs`: добавлена отдельная мобильная fullscreen-страница септаккордов без прокрутки — верхняя строка звука (громкость, угасание, смесь обертонов, детюн), левый столбец из шести типов септаккордов, центральная клавиатура и нижний выбор `linear / piano / bayiano / bayiano4`; UI «Голоса» и «Артикуляция» скрыт, дефолт `keyboardMode = holdPoly`, угасание стартует с 30 мс. Навигация и документация обновлены: `index.html`, `web/stranichki.html`, `site-nav.mjs`, `docs/architecture.md`, `docs/synth-ui.md`, `docs/synth-structure.md`; `verify-http` проверяет новую страницу. Дополнительно исправлена первопричина ERR-008 в desktop/mobile септаккордах: служебные `mapKey` добавочных голосов теперь заканчиваются реальными `note|octave`, чтобы `ToneGen.updateAllPlaying()` мог обновлять звучащие голоса. Проверка: `node --check app/seventh-chords-mobile.mjs`, `node --check app/seventh-chords.mjs`, `npm run verify`, `npm run verify:http` OK.

#082 | 2026-04-16 — DONE | `app/circle-scales.html` / `app/circle-scales.mjs` / `app/circle-scales.css` / `app/circle-of-fifths.js` / `app/circle-of-fifths.css`: подписи аккордов на круге сделаны невыделяемыми для mobile long-press; добавлена фиксированная красная кнопка `STOP` поверх страницы, которая останавливает круг и клавиатуры и возвращает клавиатуры в режим удержания; поле `Октава корня триады` → `Октава звучания круга`, дефолт октавы триады `4` → `3`. Документация: `docs/overview.md`, `docs/music-theory.md`, `docs/synth-structure.md`. Проверка: `npm run verify` OK.

#081 | 2026-04-16 — DONE | `app/circle-scales.html`: переписаны заголовок страницы, верхнее и нижнее пояснение и подписи режима клика по кругу более понятным языком; `Круг и тональность` → `Кварто-квинтовый круг и тональности`, `Тональность (подсветка на круге)` → `Выбор тональности`, кнопки режима клика → `Выделить тональность` / `Озвучить аккорд`. Проверка: `npm run verify` OK.

#080 | 2026-04-16 — DONE | `for_dany.html` оформлена в духе Undertale: тёмная пиксельная тема, шрифт **Press Start 2P**, локальные SVG-ассеты `undertale-logo`, `undertale-red-soul`, `undertale-flowey`, декоративные рамки и кнопки без изменения пяти `tg://proxy`-ссылок. Проверка: ручная сверка HTML, `rg` подтвердил `data-site-nav-theme="dark"`, шрифт и все три undertale-ассета в странице.
...
#076 | 2026-04-11 — DONE | Синхронизация репозитория: коммит `df1df64`, push `main` → `origin/main` (все накопленные изменения в каталоге `music/`).

#074 | 2026-04-11 — DONE | Подписи клавиатуры на `lads`/`lads2` по **системе знаков** выбранного лада: `CHROMATIC_NAMES_*_BY_PC`, `accidentalSystemFromScale`, `chromaticNamesByAccidentalSystem` в `lib/music-theory.js`; опции в `keyboard-layouts.mjs`, `bayan-keyboard.mjs`, `namesByPc` в `createPianoCodeMap`/`createBayanCodeMap`; `rebuildScale` → пересборка клавиатуры + `bindKeys` + подсветка; тоника с клавиши без принудительного `CANONICAL_TONIC_BY_PC`; `getBayanCodeMap` на ладах; `docs/domain.md`, `docs/music-theory.md`; проверки в `verify-theory.js`. Проверка: `node scripts/verify-theory.js`, `node --check` на затронутых модулях OK.

#075 | 2026-04-11 — DONE | **`buildScaleDegreeRowsByMidiOrder`**: ряд ступеней для арпеджио — все ноты лада в MIDI-полосе **C(n_min)…B(n_max)** как у клавиатуры (раньше якорь на тонике отрезал бас и давал несоответствие верха); `music-theory.js`, `docs/music-theory.md`, ERR-007, тесты `verify-theory.js` (G мажор с **C3**), пересборка `zigzag-sequences.jsonl` / отчёт. Проверка: `npm run verify`, `npm run zigzag:gen`.

#074 | 2026-04-11 — DONE | Снята отладочная инструментирование: `fetch` к debug-ingest и `#region agent log` из `keyboard-theory-highlight.mjs`, `lads.mjs`, `lads2.mjs`, `keyboard-synth-controller.mjs`, `circle-scales.mjs`, `circle-scales.html`. Проверка: `npm run verify`, grep по `7938/ingest` — пусто.

#073 | 2026-04-11 — DONE | Подсветка текущей ноты арпеджио / `ntg-key-active`: **`findPlayKeyElementByPitch`** (`keyboard-theory-highlight.mjs`) — совпадение клавиши по pc+октаве вместо строки имени (энгармоника D# vs Eb на пиано); `lads.mjs`, `lads2.mjs`, `keyboard-synth-controller.mjs`; ERR-006 в `docs/errors.md`. Проверка: секвенция E-мажор piano — подсветка шага на Eb/Ab при звучании D#/G#.

#072 | 2026-04-11 — DONE | Зигзаг-секвенции по 3/4/5: скрипты `scripts/generate-zigzag-sequences.mjs` → `output/zigzag-sequences.jsonl`, `scripts/analyze-zigzag-sequences.mjs` → `output/zigzag-analysis-report.md` + общая логика `scripts/zigzag-sequence-lib.mjs`; исправлена первопричина «паразитных» нот (базовый ряд ступеней с фиксированной октавой цикла давал неверный MIDI для энгармоники) — **`buildScaleDegreeRowsByMidiOrder`** в `lib/music-theory.js`, подключено в `app/lads.mjs`, `app/lads2.mjs`; ERR-004 в `docs/errors.md`, раздел секвенций в `docs/music-theory.md`, проверка в `scripts/verify-theory.js`; npm-скрипты `zigzag:gen` / `zigzag:analyze`. Проверка: `npm run verify`, `npm run zigzag:gen`, `npm run zigzag:analyze` OK

#069 | 2026-04-11 — DONE | Колонка `.cts-tone-rail`: `app/cts-tone-rail.mjs` (`mountCtsToneRail`) — шапка «Спрятать»/«Показать», `localStorage` ключ `music.cts-tone-rail.collapsed`; `circle-scales.css` — узкий viewport: `flex` + `order` у `.cts-main-column` (основной контент сверху), стили chrome/collapsed; вызов монтирования в `circle-scales.mjs`, `intervals-demo.mjs`, `seventh-chords.mjs`, `lads.mjs`, `lads2.mjs`; `docs/architecture.md`. Проверка: `npm run verify`, `node --check app/cts-tone-rail.mjs` OK

#071 | 2026-04-11 — DONE | `lads2.html`: выбранная строка таблиц — яркая рамка (боковые 3px, верх/низ 2px, лёгкий inset), усилен hover. Проверка: стили в странице OK

#070 | 2026-04-11 — DONE | `lads2`: подсветка ячеек — средняя таблица т/п разными цветами, 3-я #/b/·, 1-я по альтерации ноты; легенда в HTML. Проверка: `node --check app/lads2.mjs` OK

#068a | 2026-04-11 — DONE | `lads2`: ТЗ таблиц — (1) строки буквенные символы ладов от тоники (Δ, m7+6…), столбцы 1–7; (2)(3) строки I–VII номера ладов, столбцы 1–7; выбор лада кликом по строке; `music-theory.md`, подсказки HTML. Проверка: `node --check app/lads2.mjs` OK

#068 | 2026-04-11 — DONE | Страница `app/lads2.html` + `app/lads2.mjs`: три синхронизированные таблицы ладов от тоники (`getScalePatterns`, `buildScale`, порядок как `lads.mjs`), подсветка выбранного лада (строка + столбцы), заголовок над клавиатурой; меню `site-nav.mjs`, `index.html`, `web/stranichki.html`; доки `music-theory.md`, `overview.md`, `README.md`. Проверка: `npm run verify`, `node --check app/lads2.mjs`, HTTP 200 `/app/lads2.html` на static-server OK

#067 | 2026-04-02 — DONE | `site-nav.mjs`: меню — только 4 ссылки с `index.html` (круг/лады/интервалы/септаккорды), «Главная» → `index.html`; `docs/overview.md`, `README.md`, `docs/architecture.md`. Проверка: `npm run verify` OK

#067 | 2026-04-02 — DONE | Кварто-квинтовый круг: константы `CIRCLE_OF_FIFTHS_OUTER_LINE` / `CIRCLE_OF_FIFTHS_INNER_LINE` в `lib/music-theory.js`; убраны поля ввода колец и «Перерисовать» на `circle-of-fifths.html` и `circle-scales.html`; `circle-scales.mjs` / `circle-scales-core.js` рисуют из констант; дефолт режима круга — **удержание** (`hold`), UI и `syncCircleModeUi`; в режиме **фиксация** — `exclusiveSameClearsSelection` + повторный клик по активному сектору снимает выделение (`onSectorSelectionChange(..., null)`); **grey** по умолчанию на `circle-scales` и `lads` (состояние + `aria-pressed`). Документация: `overview.md`, `domain.md`, `music-theory.md`. Проверка: `npm run verify`, `node --check` для `circle-of-fifths.js` и `circle-scales-core.js` OK

#066 | 2026-04-02 — DONE | Убраны пользовательские инструкции про `npm run serve` / localhost: витрина `index.html`, `web/stranichki.html`, HTML-подсказки, `README` (таблица URL → `https://zagoguleolog.github.io/…`), `docs/overview.md`, `docs/music-theory.md`, `docs/synth-ui.md`, `docs/architecture.md`; сообщения об ошибке импорта в `note-tone-gen.mjs`, `template-synth.mjs`, `sequencer-demo.mjs`; JSDoc `lib/music-theory.js`; комментарии `site-nav.mjs`, `site-nav.css`, `static-server.js`, `verify-http.mjs`. Проверка: `npm run verify` OK
